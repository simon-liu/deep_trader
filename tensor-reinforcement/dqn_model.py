...
class DQN(object):
    ...
    def train_Q_network(self):
        ...
        if self.time_step % 1000 == 0:
            self.saver.save(
                self.session,
                'saved_networks/' + 'network' + '-dqn', global_step=self.time_step
            )
    ...
    for iter in xrange(ITERATION):
        ...
        for episode in xrange(len(data)):
            ...
            for step in xrange(STEP):
                state, action, next_state, reward, done, portfolio, portfolio_value =\
                    env_stage_data(agent, step, episode_data, portfolio, portfolio_value, True)
                total_reward += reward
            ...
            if episode % 100 == 0 and episode > 10:
                ...
                for i in xrange(10):
                    for step in xrange(STEP):
                        state, action, next_state, reward, done, portfolio, portfolio_value =\
                            env_stage_data(
                                agent, step, episode_data, portfolio, portfolio_value, True
                            )
                        total_reward += reward
                        if done:
                            break
        ...
        for episode in xrange(len(data)):
            ...
            for step in xrange(STEP):
                state, action, next_state, reward, done, portfolio, portfolio_value =\
                    env_stage_data(agent, step, episode_data, portfolio, portfolio_value, False)
                ...
                total_reward += reward
                if done:
                    episode_reward = show_trader_path(
                        action_list, episode_data, portfolio_list,
...
def env_stage_data(agent, step, episode_data, portfolio, portfolio_value, train):
    ...
    next_state, reward, done, portfolio, portfolio_value =\
        new_stage_data(
            action, portfolio, state, new_state,
